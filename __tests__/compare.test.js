...
test('get difference between two JSON files', () => {
  expect(
...
test('get difference between two YAML files', () => {
  expect(
    generateDiff(getFixturePath('file1.yaml'), getFixturePath('file2.yaml')),
  ).toEqual(
    '{\n - follow: false\n   host: hexlet.io\n - proxy: 123.234.53.22\n - timeout: 50\n + timeout: 20\n + verbose: true\n}',
  );
  expect(
    generateDiff(getFixturePath('file1.yml'), getFixturePath('file2.yaml')),
  ).toEqual(
    '{\n - follow: false\n   host: hexlet.io\n - proxy: 123.234.53.22\n - timeout: 50\n + timeout: 20\n + verbose: true\n}',
  );
  expect(() => {
    generateDiff(
      getFixturePath('empty-file1.yaml'),
      getFixturePath('file2.yaml'),
    );
  }).toThrow('empty-file1.yaml is empty');
});
